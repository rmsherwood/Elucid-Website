...
function gavias_sliderlayer_slides($vars){
  ...
  $vars['attributes_array']['class'] = 'gavias_sliderlayer rev_slider_wrapper ';
  $style = array();
  ...
  foreach($slides as $slide){
    if($slide->status){
      ...
      $vars['content'] .= gavias_sliderlayer_slide($slide_settings);
    }  
  }
...
function gavias_sliderlayer_slide($vars){
  global $base_url;
  $module_path = drupal_get_path('module', 'gavias_sliderlayer');
  $slide = $vars['slide'];
  $layers = $slide->layers;
  ...
  if($slide->background_image_uri){
    $vars['content'] = "<img src=\"{$path_image}{$slide->background_image_uri}\" alt=\"\"  data-duration=\"{$settings->delay}\"  data-scalestart=\"{$slide->scalestart}\" data-scaleend=\"{$slide->scaleend}\" data-kenburns=\"on\"  data-bgrepeat=\"{$slide->background_repeat}\" style=\"background-color:{$slide->background_color}\" data-bgfit=\"{$slide->background_fit}\" data-bgposition=\"{$slide->background_position}\" />";
  }else{
    $vars['content'] = "<img src=\"{$path_image}/{$module_path}/vendor/revolution/assets/transparent.png\" data-bgrepeat=\"repeat\" style=\"background-color:{$slide->background_color}\" />";
  }
